...
def install_secondary():
    ...
    try:
        dossie_v303.install(bot)
        bot._V159_RENDER_PDF_APROVADO=lambda dados,caminho: dossie_v303.gerar_pdf_dossie(bot,dados,caminho)
        bot._V155_GERAR_PDF_BASE=lambda dados,caminho: dossie_v303.gerar_pdf_dossie(bot,dados,caminho)
    except Exception as exc: diagnostic("dossie_v303",exc)
...
async def main():
    ...
    async def ready_listener():
        ...
        ready_once=True; print("✅ DISCORD READY — DICOR Discord-only ativo. Dossiê V303 ativo.",flush=True); trim_cache(); asyncio.create_task(after_ready(),name="dicor-after-ready")

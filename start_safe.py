# -*- coding: utf-8 -*-
"""Entrypoint de produção do DICOR com boot protegido e foco exclusivo no Discord."""
import asyncio
import gc
import json
import os
...
def install_compat():
    ...
    def remove_view(view):
        try:
            ...
            diagnostic("remove_view_compat", exc)
        return None
    try:
        client.remove_view = remove_view
    except Exception as exc:
        diagnostic("remove_view_install", exc)
...
def lazy_install_secondary():
    ...
    installers = (("dossie_v161", "dossie_v161", "dossie_v161_signatures", "lazy_dossie"), ("procurados_central_v162", "procurados_central_v162", None, "lazy_procurados"), ("interaction_fix_v169", "interaction_fix_v169", None, "lazy_v169"), ("pericia_fix_v181", "pericia_fix_v181", None, "lazy_v181"), ("pericia_fix_v186", "pericia_fix_v186", None, "lazy_v186"))
    for _, module_name, companion, context in installers:
        try:
            ...
            module.install(bot)
            print(f"✅ {module_name} carregado após READY.", flush=True)
        except Exception as exc:
            diagnostic(context, exc)
    try:
        import dossie_v161
        if hasattr(bot, "_V159_RENDER_PDF_APROVADO") and getattr(bot, "_V159_RENDER_PDF_APROVADO", None) is None:
            bot._V159_RENDER_PDF_APROVADO = lambda dados, caminho: dossie_v161.gerar_pdf_dossie(bot, dados, caminho)
        if hasattr(bot, "_V155_GERAR_PDF_BASE") and getattr(bot, "_V155_GERAR_PDF_BASE", None) is None:
...
async def after_ready():
    ...
    for module_name, context in (("stability_v184", "post_v184"), ("pericia_fix_v185", "post_v185")):
        try:
            __import__(module_name).install(bot)
        except Exception as exc:
            diagnostic(context, exc)
    try:
        ...
        client = getattr(bot, "bot", None)
        topic = client.get_channel(1541978969035771916) if client else None
...
async def main():
    ...
    try:
        ...
        bo_painel_fix_v202.install(bot)
    except Exception as exc:
        diagnostic("V202_bo_painel", exc)
    try:
    ...
    async def ready_listener():
        ...
        if ready_once:
            return
        ready_once = True
        print("✅ DISCORD READY — modo Discord-only ativo.", flush=True)
        trim_cache()

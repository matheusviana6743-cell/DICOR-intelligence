...
def diagnostic(context,exc):
 try:
  d=Path(str(getattr(bot,"DATA_DIR",Path(__file__).parent/"data")));d.mkdir(parents=True,exist_ok=True)
  with (d/"diagnostico_erros.jsonl").open("a",encoding="utf-8") as f:f.write(json.dumps({"timestamp":datetime.now(timezone.utc).isoformat(),"contexto":context,"tipo":type(exc).__name__,"erro":str(exc),"traceback":traceback.format_exc()},ensure_ascii=False)+"\n")
...
def trim_cache():
 try:
  ...
  if state is not None:
   state.max_messages=5;msgs=getattr(state,"_messages",None)
...
def install_compat():
 ...
 def remove_view(view):
  try:
   store=getattr(getattr(client,"_connection",None),"_view_store",None);rem=getattr(store,"remove_view",None)
   return rem(view) if callable(rem) else None

...
Painel limpo e persistente + hierarquia baseada nos membros que realmente
possuem os cargos no Discord. Não depende de IDs fixos para descobrir membros.
"""
...
# Canal do painel antigo; se não existir, resolvemos pelo nome.
...
def _role_matches(role: Any, rank: str) -> bool:
    return _rank(role) == rank


def _find_rank_role(guild: Any, rank: str) -> Optional[Any]:
    candidates = [r for r in list(getattr(guild, "roles", []) or []) if _role_matches(r, rank)]
...
def _manager(member: Any, bot_module: Any) -> bool:
    ...
    for role in list(getattr(member, "roles", []) or []):
        name = _norm(getattr(role, "name", ""))
        if "vice diretor" in name or "vice diretor dicor" in name or "inspetor" in name or "diretor dicor" in name or "dicor diretor" in name:
            return True
    return False


def _channel_by_name(client: Any, guild: Any, names: set[str]) -> Optional[Any]:
...
async def _get_panel_channel(client: Any, guild: Any) -> Optional[Any]:
    channel = client.get_channel(PANEL_CHANNEL_ID)
    if channel is not None and _norm(getattr(channel, "name", "")) in {_norm(x) for x in PANEL_NAMES}:
        return channel
    return _channel_by_name(client, guild, PANEL_NAMES)
...
async def _get_hierarchy_channel(client: Any, guild: Any) -> Optional[Any]:
    ...
    try:
        import os
        env_configured = int(str(os.getenv("DICOR_HIERARQUIA_CHANNEL_ID", "0") or "0"))
    except Exception:
        pass
    for channel_id in (env_configured, configured):
        if not channel_id:
            continue
        ...
        if channel is None:
            try:
                ...
                channel = None
        if channel is not None and _norm(getattr(channel, "name", "")) not in {_norm(x) for x in PANEL_NAMES}:
            return channel
    return _channel_by_name(client, guild, HIERARCHY_NAMES)
...
def _find_named_role(guild: Any, include: tuple[str, ...], exclude: tuple[str, ...] = ()) -> Optional[Any]:
    ...
    for role in list(getattr(guild, "roles", []) or []):
        ...
        if all(part in name for part in wanted) and not any(part in name for part in blocked):
            exact.append(role)
    return max(exact, key=lambda r: int(getattr(r, "position", 0) or 0), default=None)
...
def _hierarchy_embed(guild: Any) -> discord.Embed:
    embed = discord.Embed(
        ...
        description="Estrutura atual do efetivo. Os ocupantes são lidos diretamente dos cargos do Discord.",
        colour=discord.Colour.gold(),
    )
    embed.add_field(
        name="👑 ALTO COMANDO",
        value=(
            _members_block(guild, "🥇 Delegado Geral", ("delegado", "geral"), ("adjunto",))
            + "\n\n"
...
async def refresh_hierarchy(guild: Any) -> None:
    ...
    if channel is None:
        print("⚠️ [GESTAO V3] canal de hierarquia não encontrado.", flush=True)
        return
    ...
    try:
        async for message in channel.history(limit=80):
            if getattr(getattr(message, "author", None), "id", None) != getattr(client.user, "id", None):
                continue
            is_v3 = any(HIERARCHY_MARKER in str(getattr(getattr(e, "footer", None), "text", "")) for e in getattr(message, "embeds", []) or [])
            title_match = any("HIERARQUIA OFICIAL" in str(getattr(e, "title", "")) for e in getattr(message, "embeds", []) or [])
            content_match = "HIERARQUIA OFICIAL" in str(getattr(message, "content", ""))
            if is_v3 or title_match or content_match:
                await message.edit(content="", embed=embed, allowed_mentions=discord.AllowedMentions.none())
                return
        await channel.send(embed=embed, allowed_mentions=discord.AllowedMentions.none())
    except Exception as exc:
        print(f"⚠️ [GESTAO V3] atualização da hierarquia: {type(exc).__name__}: {exc}", flush=True)
...
def _schedule_refresh(guild: Any) -> None:
    ...
    _REFRESH_TASKS[gid] = asyncio.create_task(runner(), name=f"dicor-hierarchy-v3-{gid}")
...
def _panel_embed() -> discord.Embed:
    embed = discord.Embed(
        title="🔐 GESTÃO DICOR",
        description=(
            "Painel oficial para movimentação da equipe.\n\n"
            "🧑‍🎓 **Estagiário** → 🕵️ **Investigador** → 🛡️ **Inspetor** → 🥈 **Vice-Diretor**\n\n"
...
async def _change(interaction: Any, action: str, member: Any) -> None:
    # Reaproveita a lógica de alteração já testada no V2.
    import gestao_v2
    await gestao_v2._do_change(interaction, action, member, _BOT_MODULE)
    _schedule_refresh(getattr(interaction, "guild", None))
    ...
    async def callback(self, interaction: Any) -> None:
        try:
            ...
            try:
                ...
                pass
            print(f"⚠️ [GESTAO V3] select: {type(exc).__name__}: {exc}", flush=True)
...
async def _upgrade_panel(bot_module: Any) -> None:
    ...
    for guild in list(getattr(client, "guilds", []) or []):
        channel = await _get_panel_channel(client, guild)
        if channel is None:
            continue
        try:
            async for message in channel.history(limit=80):
                ...
                if PANEL_MARKER in content or "DICOR_GESTAO_PAINEL" in content or any(x.startswith("dicor:gestao:") for x in ids):
                    await message.edit(content=PANEL_MARKER, embed=_panel_embed(), view=GestaoV3Painel(), allowed_mentions=discord.AllowedMentions.none())
                    return
            await channel.send(content=PANEL_MARKER, embed=_panel_embed(), view=GestaoV3Painel(), allowed_mentions=discord.AllowedMentions.none())
            return
        except Exception as exc:
            print(f"⚠️ [GESTAO V3] painel: {type(exc).__name__}: {exc}", flush=True)
...
async def _on_member_update(before: Any, after: Any) -> None:
    try:
        ...
        print(f"⚠️ [GESTAO V3] member update: {type(exc).__name__}: {exc}", flush=True)
...
async def install(bot_module: Any) -> bool:
    ...
    try:
        ...
        pass
    _INSTALLED = True
    asyncio.create_task(_upgrade_panel(bot_module), name="dicor-gestao-v3-panel")
    for guild in list(getattr(client, "guilds", []) or []):
        _schedule_refresh(guild)
    print("✅ [GESTAO V3] painel limpo + hierarquia dinâmica por cargos reais ativos.", flush=True)
    return True

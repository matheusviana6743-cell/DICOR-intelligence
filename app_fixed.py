...
DB = DATA / 'lastro.db'; MAX_UPLOAD = 8 * 1024 * 1024
ALLOWED = {'jpg','jpeg','png','webp'}
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY') or secrets.token_hex(32)
app.config.update(MAX_CONTENT_LENGTH=MAX_UPLOAD, SESSION_COOKIE_HTTPONLY=True, SESSION_COOKIE_SAMESITE='Lax', SESSION_COOKIE_SECURE=os.getenv('COOKIE_SECURE','1') == '1')
...
def init_db():
    ...
    if c.execute('SELECT COUNT(*) n FROM users').fetchone()['n']==0:
        u,p=os.getenv('ADMIN_USER'),os.getenv('ADMIN_PASSWORD')
        if u and p: c.execute("INSERT INTO users(name,username,password_hash,role,active,created_at) VALUES(?,?,?,?,1,?)",('Administrador',u,generate_password_hash(p),'GERENCIA',now())); c.commit()
    c.close()
def csrf():
...
@app.before_request
def before(): init_db(); check_csrf()
def login_required(f):
    @wraps(f)
    def w(*a,**k):
        return redirect(url_for('login')) if not session.get('user_id') else f(*a,**k)
    return w
...
def save_proof(f):
    ...
    name=f'{uuid.uuid4().hex}.{ext}'; p=UPLOADS/name; f.save(p)
    if p.stat().st_size>MAX_UPLOAD: p.unlink(missing_ok=True); abort(413)
    return name
def proof_link(name,label='abrir'):
    if not name: return '—'
    return f'<a class="proof" href="{url_for("proof",filename=name)}" target="_blank" rel="noopener">{label}</a>'
def member_options(): return rows('SELECT id,name,cargo FROM members WHERE active=1 ORDER BY name')

CSS='''
:root{--bg:#080b10;--panel:#11161f;--line:#252d39;--text:#f4f7fa;--muted:#8d98a8;--accent:#7c5cff;--accent2:#aa98ff;--ok:#35d58b;--danger:#ff5d73}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 85% -15%,#24194d 0,#080b10 38%),var(--bg);color:var(--text);font:14px Inter,system-ui,-apple-system,Segoe UI,sans-serif}a{text-decoration:none;color:inherit}.app{display:flex;min-height:100vh}.side{width:248px;background:#0b0f15f2;border-right:1px solid var(--line);padding:22px 14px;position:fixed;inset:0 auto 0 0}.brand{padding:8px 12px 26px}.brand b{font-size:23px;letter-spacing:.1em}.brand span{display:block;color:var(--muted);font-size:10px;margin-top:5px;letter-spacing:.08em}.nav a{display:flex;gap:11px;padding:11px 12px;border-radius:10px;color:#aeb7c5;margin:4px 0}.nav a:hover{background:#181e29;color:#fff}.main{margin-left:248px;width:calc(100% - 248px);padding:28px 34px}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:25px}.top h1{margin:0;font-size:26px}.user{color:var(--muted);font-size:12px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.card{background:linear-gradient(180deg,#151a23,#10141b);border:1px solid var(--line);border-radius:16px;padding:18px}.metric{font-size:28px;font-weight:750;margin-top:8px}.label{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.08em}.section{margin-top:18px}.section h2{font-size:17px}.table-wrap{overflow:auto}.table{width:100%;border-collapse:collapse}.table th,.table td{padding:12px 10px;border-bottom:1px solid var(--line);text-align:left;white-space:nowrap}.table th{color:#8e99a9;font-size:10px;text-transform:uppercase}.btn{border:0;border-radius:10px;padding:10px 14px;background:var(--accent);color:#fff;font-weight:650;cursor:pointer}.btn.secondary{background:#202631}.form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.field{display:flex;flex-direction:column;gap:6px}.field.full{grid-column:1/-1}.field label{font-size:12px;color:#aeb7c5}.input,.select,.textarea{width:100%;background:#0a0e14;border:1px solid var(--line);border-radius:10px;padding:11px 12px;color:#fff;outline:none}.textarea{min-height:90px;resize:vertical}.input:focus,.select:focus,.textarea:focus{border-color:var(--accent)}.flash{padding:11px 13px;border-radius:10px;background:#18271f;border:1px solid #275c42;margin-bottom:14px}.flash.error{background:#321820;border-color:#6e2c38}.login{min-height:100vh;display:grid;place-items:center;padding:20px}.loginbox{width:min(420px,100%);padding:30px}.muted{color:var(--muted)}.pill{display:inline-block;padding:5px 8px;border-radius:999px;background:#202632;color:#c5ccd6;font-size:11px}.proof{color:var(--accent2)}.empty{padding:28px;text-align:center;color:var(--muted)}.mobile-nav{display:none}@media(max-width:900px){.side{display:none}.main{margin:0;width:100%;padding:18px 14px 82px}.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.form{grid-template-columns:1fr}.mobile-nav{display:flex;position:fixed;z-index:20;bottom:0;left:0;right:0;background:#0c0f15f5;border-top:1px solid var(--line);padding:7px;gap:4px;overflow:auto}.mobile-nav a{min-width:70px;text-align:center;font-size:10px;color:#9da7b7;padding:7px}.mobile-nav a b{display:block;font-size:16px;margin-bottom:2px}.top h1{font-size:21px}}
'''
ICONS={'Painel':'◈','Farm':'⬢','Produção':'◉','Baú':'▣','Ações':'◇','Ranking':'↗','Hierarquia':'♟','Usuários':'⚙'}
def layout(title,body):
    items=[('Painel','dashboard'),('Farm','farms'),('Produção','productions'),('Baú','chests'),('Ações','actions'),('Ranking','ranking'),('Hierarquia','members'),('Usuários','users')]
    nav=[]
    for n,r in items:
        if n=='Usuários' and session.get('role')!='GERENCIA': continue
        nav.append(f'<a href="{url_for(r)}"><b>{ICONS[n]}</b>{n}</a>')
    flashes=''.join(f'<div class="flash {"error" if c=="error" else ""}">{m}</div>' for c,m in session.pop('_flashes',[]))
    return render_template_string(f'''<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#080b10"><title>{title} · CENTRAL LASTRO</title><style>{CSS}</style></head><body><div class="app"><aside class="side"><div class="brand"><b>LASTRO</b><span>CENTRAL ADMINISTRATIVA</span></div><nav class="nav">{''.join(nav)}</nav><div style="position:absolute;bottom:20px;left:26px;right:26px"><div class="muted" style="font-size:11px;margin-bottom:10px">{session.get('name','')}</div><a class="btn secondary" style="display:block;text-align:center" href="{url_for('logout')}">Sair</a></div></aside><main class="main"><div class="top"><h1>{title}</h1><div class="user">{session.get('role','')}</div></div>{flashes}{body}</main><nav class="mobile-nav">{''.join(nav)}</nav></div></body></html>''')
...
def login():
    ...
    if request.method=='POST':
        u=request.form.get('username','').strip(); p=request.form.get('password',''); r=rows('SELECT * FROM users WHERE username=? AND active=1',(u,))
        if r and check_password_hash(r[0]['password_hash'],p):
            session.clear(); session.update(user_id=r[0]['id'],name=r[0]['name'],role=r[0]['role']); csrf(); return redirect(url_for('dashboard'))
...
@login_required
def dashboard():
    tf=rows('SELECT COALESCE(SUM(quantity),0) v FROM farms')[0]['v']; tp=rows('SELECT COALESCE(SUM(quantity),0) v FROM productions')[0]['v']; tm=rows('SELECT COALESCE(SUM(value),0) v FROM action_logs')[0]['v']; mm=rows('SELECT COUNT(*) n FROM members WHERE active=1')[0]['n']
    ...
    return layout('Painel',f'<div class="grid"><div class="card"><div class="label">Farm entregue</div><div class="metric">{tf:g}</div></div><div class="card"><div class="label">Produzido</div><div class="metric">{tp:g}</div></div><div class="card"><div class="label">Ações movimentadas</div><div class="metric">{money(tm)}</div></div><div class="card"><div class="label">Membros ativos</div><div class="metric">{mm}</div></div></div><div class="card section"><h2>Atividade recente</h2><div class="table-wrap"><table class="table"><tr><th>Tipo</th><th>Pessoa</th><th>Quantidade/Valor</th><th>Data</th></tr>{trs}</table></div></div>')
@app.route('/farms',methods=['GET','POST'])
@login_required
def farms():
    if request.method=='POST':
        proof=save_proof(request.files.get('proof')); execute('INSERT INTO farms(member_id,quantity,proof_path,created_by,created_at) VALUES(?,?,?,?,?)',(request.form['member_id'],float(request.form['quantity']),proof,session['user_id'],now())); flash('Registro de farm salvo.')
    ...
    trs=''.join('<tr><td>{}</td><td>{:g}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(r['member'],r['quantity'],fmt_dt(r['created_at']),r['creator'],proof_link(r['proof_path'],'abrir prova')) for r in data) or '<tr><td colspan="5" class="empty">Nenhum registro.</td></tr>'
...
def productions():
    if request.method=='POST':
        proof=save_proof(request.files.get('proof')); execute('INSERT INTO productions(member_id,quantity,material,reason,proof_path,created_by,created_at) VALUES(?,?,?,?,?,?,?)',(request.form['member_id'],float(request.form['quantity']),request.form['material'].strip(),request.form['reason'],proof,session['user_id'],now())); flash('Produção registrada.')
    ...
    trs=''.join('<tr><td>{}</td><td>{:g}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(r['member'],r['quantity'],r['material'],r['reason'],fmt_dt(r['created_at']),proof_link(r['proof_path'])) for r in data) or '<tr><td colspan="6" class="empty">Nenhum registro.</td></tr>'
...
def chests():
    if request.method=='POST':
        proof=save_proof(request.files.get('proof')); execute('INSERT INTO chests(member_id,item,reason,proof_path,created_by,created_at) VALUES(?,?,?,?,?,?)',(request.form['member_id'],request.form['item'].strip(),request.form['reason'].strip(),proof,session['user_id'],now())); flash('Retirada do baú registrada.')
    ...
    trs=''.join('<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(r['member'],r['item'],r['reason'],fmt_dt(r['created_at']),proof_link(r['proof_path'])) for r in data) or '<tr><td colspan="5" class="empty">Nenhum registro.</td></tr>'
    opts=''.join(f'<option value="{m["id"]}">{m["name"]}</option>' for m in member_options())
...
def actions():
    ...
    ltr=''.join('<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(l['action'],l['member'],money(l['value']),fmt_dt(l['created_at']),proof_link(l['proof_path'])) for l in logs) or '<tr><td colspan="5" class="empty">Nenhuma participação.</td></tr>'
...
def action_detail(action_id):
    ...
    trs=''.join('<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(l['member'],money(l['value']),fmt_dt(l['created_at']),proof_link(l['proof_path'])) for l in logs) or '<tr><td colspan="4" class="empty">Sem participações.</td></tr>'
...
@login_required
def ranking():
    farm=rows('SELECT m.name,COALESCE(SUM(f.quantity),0) total FROM members m LEFT JOIN farms f ON f.member_id=m.id WHERE m.active=1 GROUP BY m.id ORDER BY total DESC,m.name'); act=rows('SELECT m.name,COUNT(l.id) total FROM members m LEFT JOIN action_logs l ON l.member_id=m.id WHERE m.active=1 GROUP BY m.id ORDER BY total DESC,m.name')
